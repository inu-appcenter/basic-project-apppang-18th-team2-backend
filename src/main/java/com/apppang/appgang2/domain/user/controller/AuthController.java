...
public class AuthController {
    ...
    //회원가입
    @PostMapping("/signup")
    public ResponseEntity<ApiResponse<Map<String,Long>>> signup(@Valid @RequestBody SignupDTO signupDTO){

        //비즈니스 로직 완료 후 응답에 필요한 userId만 서비스로부터 받아옴
        Long savedUserId = authService.signup(signupDTO);

        //API 명세서 "data" 구조를 맞추기 위해 반환받은 savedUserId를 userId 이름을 붙여 응답 객체 생성
        Map<String, Long> data = new HashMap<>();
        data.put("userId", savedUserId);
